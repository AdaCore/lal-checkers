...
def gen_ir(ctx, subp, typer, subpdata):
    ...
    def gen_for_condition(iter_var, iter_expr, iter_type):
        ...
        if iter_expr.is_a(lal.DiscreteSubtypeIndication):
            ...
            ref = iter_expr.p_referenced_decl(True)
    ...
    def gen_case_condition(expr, choices):
        ...
        def gen_single(choice):
            ...
            if (choice.is_a(lal.Identifier)
                    and choice.p_referenced_decl(True).is_a(lal.SubtypeDecl)):
                try:
                    actual_type = typer.get(choice.p_referenced_decl(True))
                    if actual_type.is_a(types.IntRange):
                        return gen_range(
                            actual_type.frm, actual_type.to,
                            None, None
    ...
    def gen_cast_expr(expr, tpe, orig_node):
        ...
        # todo: implement cast
        return [], stores.register_normal(
            name="unimpl", orig_node=orig_node, type_hint=tpe
        ).get()
    ...
    def gen_actual_dest(dest, expr):
        ...
        if dest.is_a(lal.Identifier, lal.DefiningName):
            ...
            if dest.f_name.p_referenced_decl(True).is_a(lal.TypeDecl):
                # type conversion
                cast_pre_stmts, cast_expr = gen_cast_expr(
                    expr,
                    dest.f_name.p_referenced_decl(True),
                    dest
                )
                dest_pre_stmts, dest = gen_actual_dest(
                    dest.f_suffix[0].f_r_expr,
                    cast_expr
                )
                return cast_pre_stmts + dest_pre_stmts, dest
    ...
    def gen_field_existence_condition(prefix, field):
        ...
        all_fields = record_fields(
            closest(field.p_referenced_decl(True), lal.TypeDecl)
        )
    ...
    def transform_stmt(stmt):
        ...
        if stmt.is_a(lal.AssignStmt):
            ...
            label = labels[stmt.f_label_name.p_referenced_decl(True)]
            ...
            if stmt.f_loop_name is None:
                ...
                named_loop_decl = stmt.f_loop_name.p_referenced_decl(True)
